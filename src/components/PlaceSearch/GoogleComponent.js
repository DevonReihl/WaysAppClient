...
export class GoogleComponent extends Component {
  ...
  async componentDidMount() {
    await this.setState({
      liStyle: this.props.locationListStyle
        ? this.props.locationListStyle
        : 'style-list',
    });
  ...
  getInfo(param) {
    let child = [];
    let _co = this.props.country
      ? 'components=' + this.props.country + '&'
      : '';
    let _lang = this.props.language
                ...
                child.push(
                  React.createElement(
                    'li',
                    {
                      ...
                      className: this.state.liStyle,
                      onClick: () =>
                        this.arrangeList(data.predictions[loc].description),
                    },
  ...
  async _returnData(place) {
    ...
    if (this.props.coordinates) {
      if (location.status === 'OK') {
        let returnData = this.state.returnData;
        returnData.coordinates = location.results[0].geometry.location;
        this.setState({ returnData });
      } else {
  ...
  render() {
    return React.createElement(
      ...
      React.createElement('input', {
        type: 'text',
        className: this.props.locationBoxStyle
          ? this.props.locationBoxStyle
          : 'location-box',
